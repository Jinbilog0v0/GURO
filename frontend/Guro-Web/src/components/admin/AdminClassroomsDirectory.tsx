...
import { School, Search, Lock, Unlock, Users, Trash2, Edit3, Calculator, BookOpen, RefreshCw, X, Calendar } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-1.5 font-bold text-xs text-[var(--text-main)]">
                            ...
                            {c.sectionName ? (
                              <span className="text-[10px] bg-[#11428E]/10 text-[#11428E] px-1.5 py-0.5 rounded font-mono">
                                Sec: {c.sectionName}
                              </span>
                            ) : null}
                          </div>
                          <div className="flex items-center gap-1 text-[10px] text-[var(--text-muted)] font-medium">
                            ...
                            <span>S.Y. {c.schoolYear || '2026-2027'} • {c.term || 'Quarter 1'}</span>
                          </div>
                        </div>
                      </td>
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl p-6 max-w-sm w-full shadow-2xl flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-[#11428E] dark:text-blue-400">
                <Edit3 size={18} />
                <h4 className="font-extrabold text-base text-[var(--text-main)] m-0">Reassign Teacher</h4>
              </div>
              <button
      ...
      <ConfirmModal
        ...
        description={
          deletingClassroom
            ? `Are you sure you want to archive and remove classroom section "${deletingClassroom.classroomId}" (${deletingClassroom.subject} Grade ${deletingClassroom.gradeLevel})?`
            : ''
        }
