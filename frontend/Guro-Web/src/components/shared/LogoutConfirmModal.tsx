...
export const LogoutConfirmModal: React.FC<LogoutConfirmModalProps> = ({ isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] transition-opacity duration-300"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-3xl p-6 max-w-md w-full mx-4 shadow-2xl transform scale-100 transition-transform duration-300">
        ...
        <div className="flex items-center justify-center w-14 h-14 bg-[#A01322]/10 border border-[#A01322]/20 rounded-full mx-auto mb-4">
          <LogOut className="w-6 h-6 text-[#A01322]" aria-hidden="true" />
        </div>

        <h3 id="logout-modal-title" className="text-xl font-extrabold text-zinc-100 text-center mb-2 tracking-tight">
          Confirm Logout
        </h3>
        <p id="logout-modal-desc" className="text-zinc-400 text-sm text-center mb-6 leading-relaxed">
